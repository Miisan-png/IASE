...
class SplashScreen:
    ...
    def load_images(self):
        powered_by_path = os.path.join("Assets", "Util", "powered_by.png")
        engine_logo_path = os.path.join("Assets", "Util", "engine_logo.png")
        
        if os.path.exists(powered_by_path):
            original_img = pygame.image.load(powered_by_path).convert_alpha()
            self.powered_by_img = self.scale_image_to_fit(original_img)
        else:
            self.powered_by_img = pygame.Surface((200, 100))
            self.powered_by_img.fill((255, 255, 255))
    ...
    def render(self):
        ...
        if self.state in ["fade_in_powered", "display_powered", "fade_out_powered"]:
            current_img = self.powered_by_img
        else:
